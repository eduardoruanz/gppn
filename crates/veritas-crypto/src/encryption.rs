...
impl EncryptedPayload {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, CryptoError> {
        if bytes.len() < 44 {
            return Err(CryptoError::DecryptionError("payload too short".into()));
        }
...
fn derive_x25519_secret(keypair: &KeyPair) -> StaticSecret {
    ...
    let derived = blake3::derive_key("Veritas-x25519-key-derivation-v1", &seed);
...
pub fn encrypt(
    ...
    let symmetric_key = blake3::derive_key("Veritas-encryption-v1", shared_secret.as_bytes());
...
mod tests {
    ...
    fn test_encrypt_decrypt_roundtrip() {
        ...
        let plaintext = b"Hello, Veritas payment metadata!";
