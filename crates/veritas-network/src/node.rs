//! The Veritas network node.
//!
//! `VeritasNode` is the main entry point for the P2P networking layer.
...
/// Configuration for the VeritasNode.
#[derive(Debug, Clone)]
...
pub enum NetworkCommand {
    /// Broadcast a credential via gossipsub.
    BroadcastCredential {
        data: Vec<u8>,
...
pub struct VeritasNode {
    ...
    swarm: Option<Swarm<VeritasBehaviour>>,
...
impl VeritasNode {
    /// Create a new VeritasNode with the given keypair and config.
    pub fn new(keypair: Keypair, config: NodeConfig) -> Result<Self, NetworkError> {
        ...
        tracing::info!(%local_peer_id, "creating Veritas node");
    ...
    /// request actions on the swarm (e.g., broadcasting a credential).
    ...
    pub async fn start(&mut self) -> Result<Multiaddr, NetworkError> {
        ...
        // Build the swarm
        let mut swarm =
            transport::build_swarm(self.keypair.clone(), |key| VeritasBehaviour::new(key))?;

        // Subscribe to all Veritas topics
        self.topic_manager
        ...
        let actual_addr =
            Multiaddr::from_str(&self.config.listen_addr).unwrap_or_else(|_| Multiaddr::empty());
    ...
    pub async fn run(&mut self) -> Result<(), NetworkError> {
        ...
        tracing::info!(peer_id = %self.local_peer_id, "Veritas node event loop started");

        #[allow(clippy::large_enum_variant)]
        enum Action {
            SwarmEvent(SwarmEvent<VeritasBehaviourEvent>),
        ...
        loop {
            ...
            match action {
                ...
                Action::Shutdown => {
                    tracing::info!("Veritas node shutting down (signal)");
                    break;
                }
                Action::CommandChannelClosed => {
                    tracing::info!("Veritas node shutting down (command channel closed)");
                    break;
                }
            }
        }
    ...
    pub async fn stop(&mut self) -> Result<(), NetworkError> {
        ...
        tracing::info!(peer_id = %self.local_peer_id, "Veritas node stopped");
        Ok(())
    }

    /// Broadcast a credential over GossipSub.
    pub fn broadcast_credential(&mut self, data: Vec<u8>) -> Result<(), NetworkError> {
        let swarm = self.swarm.as_mut().ok_or(NetworkError::NotStarted)?;
        let topic = &self.topic_manager.credentials;
        ...
        tracing::debug!("broadcast credential to gossipsub");
        Ok(())
    }
    ...
    pub fn send_request(
        &mut self,
        peer_id: &PeerId,
        request: crate::protocol::VeritasRequest,
    ...
    fn handle_command(&mut self, cmd: NetworkCommand) {
        match cmd {
            NetworkCommand::BroadcastCredential { data, reply } => {
                let result = self.broadcast_credential(data);
                let _ = reply.send(result);
            }
            NetworkCommand::SendDirectRequest {
                peer_id,
                data,
                reply,
            } => {
                let request = crate::protocol::VeritasRequest::CredentialRequest {
    ...
    fn handle_swarm_event(&mut self, event: SwarmEvent<VeritasBehaviourEvent>) {
        match event {
            ...
            SwarmEvent::ConnectionEstablished {
                ...
                let _ = self
                    .event_tx
                    .send(NetworkEvent::PeerConnected(PeerConnected {
                        peer_id,
    ...
    fn handle_behaviour_event(&mut self, event: VeritasBehaviourEvent) {
        match event {
            VeritasBehaviourEvent::Gossipsub(gossipsub::Event::Message {
                propagation_source,
                message,
                ..
            }) => {
                ...
                if message.topic == self.topic_manager.credentials_hash() {
                    let _ =
                        self.event_tx
                            .send(NetworkEvent::IncomingCredential(IncomingCredential {
            ...
            VeritasBehaviourEvent::Gossipsub(gossipsub::Event::Subscribed { peer_id, topic }) => {
                tracing::debug!(%peer_id, %topic, "peer subscribed to topic");
            }
            VeritasBehaviourEvent::Gossipsub(gossipsub::Event::Unsubscribed { peer_id, topic }) => {
                tracing::debug!(%peer_id, %topic, "peer unsubscribed from topic");
            }
            VeritasBehaviourEvent::Gossipsub(_) => {}

            VeritasBehaviourEvent::Kademlia(kad::Event::RoutingUpdated {
                peer,
                is_new_peer,
                ..
            }) => {
            ...
            VeritasBehaviourEvent::Kademlia(kad::Event::OutboundQueryProgressed {
                result, ..
            }) => {
                tracing::debug!(?result, "kademlia query progressed");
            }
            VeritasBehaviourEvent::Kademlia(_) => {}

            VeritasBehaviourEvent::Mdns(mdns::Event::Discovered(peers)) => {
                ...
                for (peer_id, addr) in peers {
                    ...
                    if let Some(ref mut s) = swarm {
                        ...
                        s.behaviour_mut().kademlia.add_address(&peer_id, addr);
                    }
                }
            }
            VeritasBehaviourEvent::Mdns(mdns::Event::Expired(peers)) => {
            ...
            VeritasBehaviourEvent::Identify(identify::Event::Received {
                peer_id, info, ..
            }) => {
                tracing::debug!(
                    %peer_id,
            ...
            VeritasBehaviourEvent::Identify(_) => {}

            VeritasBehaviourEvent::RequestResponse(libp2p::request_response::Event::Message {
                peer,
                message,
            }) => match message {
            ...
            VeritasBehaviourEvent::RequestResponse(
            ...
            VeritasBehaviourEvent::RequestResponse(_) => {}
        }
    }
    ...
    fn handle_incoming_request(
        ...
        peer: PeerId,
        request: crate::protocol::VeritasRequest,
        channel: libp2p::request_response::ResponseChannel<crate::protocol::VeritasResponse>,
    ) {
        tracing::debug!(%peer, "incoming request");

        match request {
            crate::protocol::VeritasRequest::CredentialRequest {
                request_id, claims, ..
            } => {
                let _ = self
                    .event_tx
                    .send(NetworkEvent::DirectMessage(DirectMessage {
                        peer_id: peer,
                        data: claims,
                    }));

                let response = crate::protocol::VeritasResponse::CredentialResponse {
                    request_id,
                    issued: false,
                    credential_data: vec![],
            ...
            crate::protocol::VeritasRequest::ProofRequest {
                request_id,
                requirements,
                ..
            } => {
                ...
                let response = crate::protocol::VeritasResponse::ProofResponse {
                    request_id,
                    valid: false,
                    proof_data: vec![],
                };
            ...
            crate::protocol::VeritasRequest::DidResolve { did } => {
                let response = crate::protocol::VeritasResponse::DidDocument {
                    did,
                    document_data: vec![],
                    found: false,
                };
            ...
            crate::protocol::VeritasRequest::TrustAttestation { .. } => {
                let response = crate::protocol::VeritasResponse::TrustUpdate {
                    accepted: true,
                    new_score: None,
                };
                if let Some(ref mut swarm) = self.swarm {
            ...
            crate::protocol::VeritasRequest::Ping => {
                if let Some(ref mut swarm) = self.swarm {
                    let _ = swarm
                        .behaviour_mut()
                        .request_response
                        .send_response(channel, crate::protocol::VeritasResponse::Pong);
                }
            }
        }
    }
    ...
    fn handle_incoming_response(&self, peer: PeerId, response: crate::protocol::VeritasResponse) {
        tracing::debug!(%peer, "incoming response");

        match response {
            crate::protocol::VeritasResponse::CredentialResponse {
                request_id, issued, ..
            } => {
                tracing::debug!(
                    %peer,
            ...
            crate::protocol::VeritasResponse::ProofResponse {
                request_id, valid, ..
            } => {
                tracing::debug!(
                    %peer,
            ...
            crate::protocol::VeritasResponse::DidDocument { did, found, .. } => {
                tracing::debug!(%peer, %did, found, "DID document response received");
            }
            crate::protocol::VeritasResponse::TrustUpdate {
                accepted,
                new_score,
            } => {
                tracing::debug!(
                    ...
                    accepted,
                    ?new_score,
                    "trust update received"
                );
            }
            crate::protocol::VeritasResponse::Pong => {
                tracing::debug!(%peer, "pong received");
            }
            crate::protocol::VeritasResponse::Error { message } => {
...
mod tests {
    ...
    fn test_node_creation() {
        ...
        let node = VeritasNode::new(keypair, config);
        assert!(node.is_ok());
    }
    ...
    fn test_node_local_peer_id() {
        ...
        let config = NodeConfig::default();
        let node = VeritasNode::new(keypair, config).expect("node creation");
    ...
    #[test]
    fn test_broadcast_credential_before_start() {
        let keypair = make_keypair();
        let config = NodeConfig::default();
        let mut node = VeritasNode::new(keypair, config).expect("node creation");
        let result = node.broadcast_credential(vec![1, 2, 3]);
    ...
    fn test_send_request_before_start() {
        ...
        let result = node.send_request(&peer, crate::protocol::VeritasRequest::Ping);
        assert!(matches!(result, Err(NetworkError::NotStarted)));
    }
