//! libp2p transport stack construction for the Veritas network.
//!
...
use crate::behaviour::VeritasBehaviour;
use crate::error::NetworkError;

/// Configuration for building the Veritas network transport.
#[derive(Debug, Clone)]
...
/// Build a libp2p Swarm with the Veritas behaviour using TCP + Noise + Yamux.
///
...
pub fn build_swarm(
    keypair: Keypair,
    behaviour_fn: impl FnOnce(&Keypair) -> Result<VeritasBehaviour, NetworkError>,
) -> Result<libp2p::Swarm<VeritasBehaviour>, NetworkError> {
...
mod tests {
    ...
    fn test_build_swarm_success() {
        ...
        let result = build_swarm(keypair, |key| crate::behaviour::VeritasBehaviour::new(key));
        assert!(result.is_ok());
    }
}
