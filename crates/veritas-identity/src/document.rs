...
pub struct VerificationMethod {
    /// Verification method identifier (e.g., "did:veritas:key:abc#keys-1").
    pub id: String,
...
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct Service {
    /// Service identifier (e.g., "did:veritas:key:abc#veritas-endpoint").
    pub id: String,
    /// Service type (e.g., "VeritasNode", "VeritasRelay").
    pub service_type: String,
    /// Service endpoint URL.
    pub service_endpoint: String,
}

/// W3C-compatible DID Document for the Veritas protocol.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct DidDocument {
    /// The DID subject (e.g., "did:veritas:key:<bs58_pubkey>").
    pub id: String,
...
impl DidDocument {
    ...
    pub fn add_verification_method(&mut self, method_type: &str, public_key_bs58: &str) {
...
mod tests {
    ...
    fn test_create_did_document() {
        let doc = DidDocument::new("did:veritas:key:abc123".to_string(), "abc123".to_string());
        assert_eq!(doc.id, "did:veritas:key:abc123");
    ...
    #[test]
    fn test_add_service() {
        let mut doc = DidDocument::new("did:veritas:key:abc".to_string(), "abc".to_string());
        doc.add_service("VeritasNode", "https://node.example.com");
        assert_eq!(doc.services.len(), 1);
        assert_eq!(doc.services[0].service_type, "VeritasNode");
        assert_eq!(doc.services[0].service_endpoint, "https://node.example.com");
    }
    ...
    fn test_primary_public_key() {
        let doc = DidDocument::new("did:veritas:key:xyz".to_string(), "xyz".to_string());
        assert_eq!(doc.primary_public_key_bs58(), Some("xyz"));
    }

    #[test]
    fn test_serialization_roundtrip() {
        let mut doc = DidDocument::new("did:veritas:key:test".to_string(), "test".to_string());
        doc.add_service("VeritasNode", "https://node.example.com");
