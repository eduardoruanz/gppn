...
/// Full configuration for the GPPN node.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct GppnConfig {
...
impl GppnConfig {
    ...
    pub fn load(path: &Path) -> anyhow::Result<Self> {
        if path.exists() {
            ...
            let config: GppnConfig = toml::from_str(&contents)?;
            Ok(config)
        } else {
            Ok(Self::default())
        }
...
mod tests {
    ...
    fn test_config_serialization_roundtrip() {
        ...
        let decoded: GppnConfig = toml::from_str(&toml_str).expect("deserialize");
    ...
    fn test_config_load_nonexistent_uses_defaults() {
        let config = GppnConfig::load(Path::new("/nonexistent/config.toml")).unwrap();
        assert_eq!(config.network.port, 9000);
    }
...
port = 8001
"#;
        let config: GppnConfig = toml::from_str(toml_str).expect("parse");
