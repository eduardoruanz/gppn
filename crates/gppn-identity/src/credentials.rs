...
use gppn_crypto::{sign, verify, KeyPair, PublicKey, Signature};
use serde::{Deserialize, Serialize};
use uuid::Uuid;

use crate::error::IdentityError;

/// A W3C-inspired Verifiable Credential for the GPPN protocol.
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct VerifiableCredential {
    /// Unique credential identifier.
    pub id: String,
    /// Type(s) of the credential (e.g., ["VerifiableCredential", "GppnNodeAttestation"]).
...
pub struct CredentialProof {
    ...
    /// Verification method ID (e.g., "did:gppn:key:abc#keys-1").
...
impl VerifiableCredential {
    ...
    pub fn verify_proof(&self, public_key: &PublicKey) -> Result<(), IdentityError> {
        let proof = self
            .proof
            .as_ref()
            .ok_or_else(|| IdentityError::CredentialVerification("no proof attached".to_string()))?;
...
mod tests {
    ...
    fn test_credential(issuer: &str, subject: &str) -> VerifiableCredential {
        VerifiableCredential::new(
            issuer.to_string(),
            subject.to_string(),
            vec!["GppnNodeAttestation".to_string()],
            serde_json::json!({
                "nodeVersion": "0.1.0",
                "uptime": 0.99,
            }),
    ...
    fn test_create_credential() {
        let vc = test_credential("did:gppn:key:issuer", "did:gppn:key:subject");
        assert!(vc.id.starts_with("urn:uuid:"));
        assert!(vc.credential_type.contains(&"VerifiableCredential".to_string()));
        assert!(vc.credential_type.contains(&"GppnNodeAttestation".to_string()));
        assert!(!vc.is_signed());
    }
    ...
    fn test_verify_wrong_key() {
        ...
        assert!(matches!(result, Err(IdentityError::CredentialVerification(_))));
    }
    ...
    fn test_no_duplicate_vc_type() {
        let vc = VerifiableCredential::new(
            "did:gppn:key:issuer".to_string(),
            "did:gppn:key:subject".to_string(),
